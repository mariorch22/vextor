...
static std::vector<float> make_vector(vexdb::Dim dim, float start, float step) {
    std::vector<float> v(dim);
    for (vexdb::Dim i = 0; i < dim; i++) {
...
TEST(Distance, ScalarIdenticalVectorsIsZero) {
    ...
    EXPECT_FLOAT_EQ(vexdb::l2_distance_scalar(a.data(), a.data(), 3), 0.0f);
}
...
TEST(Distance, ScalarKnownResult) {
    ...
    EXPECT_FLOAT_EQ(vexdb::l2_distance_scalar(a.data(), b.data(), 2), 25.0f);
}

TEST(Distance, ScalarSingleDimension) {
    float a = 3.0f;
    float b = 7.0f;
    EXPECT_FLOAT_EQ(vexdb::l2_distance_scalar(&a, &b, 1), 16.0f);
}

// --- AVX2 vs Scalar consistency across dimensions ---

class DistanceConsistencyTest : public ::testing::TestWithParam<vexdb::Dim> {};
...
TEST_P(DistanceConsistencyTest, AVX2MatchesScalar) {
    ...
    float scalar = vexdb::l2_distance_scalar(a.data(), b.data(), dim);
    float avx2 = vexdb::l2_distance(a.data(), b.data(), dim);
...
TEST(Distance, ZeroDimension) {
    EXPECT_FLOAT_EQ(vexdb::l2_distance(nullptr, nullptr, 0), 0.0f);
    EXPECT_FLOAT_EQ(vexdb::l2_distance_scalar(nullptr, nullptr, 0), 0.0f);
}

TEST(Distance, LargeIdenticalVectors) {
    std::vector<float> a(768, 42.0f);
    EXPECT_FLOAT_EQ(vexdb::l2_distance(a.data(), a.data(), 768), 0.0f);
    EXPECT_FLOAT_EQ(vexdb::l2_distance_scalar(a.data(), a.data(), 768), 0.0f);
}